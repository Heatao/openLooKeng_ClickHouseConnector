...
import io.hetu.core.plugin.clickhouse.rewrite.ClickHouseSqlQueryWriter;
        ...
        extends BaseJdbcClient
{
     ...
     */
    @Inject
    public ClickHouseClient(BaseJdbcConfig config, ClickHouseConfig clickHouseConfig, @StatsCollecting ConnectionFactory connectionFactory)
    {
        super(config, "\"", connectionFactory);
        tableTypes = clickHouseConfig.getTableTypes().split(",");
        isQueryPushDownEnabled = clickHouseConfig.isQueryPushDownEnabled();
    ...
    public Optional<ColumnMapping> toPrestoType(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        if (!columnMapping.isPresent()) {
            throw new UnsupportedOperationException(
                    "Hetu does not support the clickhouse type " + typeHandle.getJdbcTypeName().orElse("")
    ...
    protected Collection<String> listSchemas(Connection connection)
    {
        try (ResultSet resultSet = connection.getMetaData().getSchemas()) {
            ...
            throw new PrestoException(JDBC_ERROR, "Hetu ClickHouse connector failed to list schemas");
        }
    }
    ...
    public void addColumn(ConnectorSession session, JdbcTableHandle handle, ColumnMetadata column)
    {
        try (Connection connection = connectionFactory.openConnection(JdbcIdentity.from(session))) {
            ...
            throw new PrestoException(JDBC_ERROR, "Hetu ClickHouse connector failed to add column, check table engine and sql");
        }
    }
    ...
    public void dropColumn(JdbcIdentity identity, JdbcTableHandle handle, JdbcColumnHandle column)
    {
        try (Connection connection = connectionFactory.openConnection(identity)) {
            ...
            throw new PrestoException(JDBC_ERROR, "Hetu ClickHouse connector failed to drop column");
        }
    }
    ...
    protected void renameTable(JdbcIdentity identity, String catalogName, String schemaName, String tableName, SchemaTableName newTable)
    {
        try (Connection connection = connectionFactory.openConnection(identity)) {
            ...
            throw new PrestoException(JDBC_ERROR, "Hetu ClickHouse connector failed to rename table");
        }
    }
    ...
    public Optional<SqlQueryWriter> getSqlQueryWriter()
    {
        ...
        return Optional.of(new ClickHouseSqlQueryWriter(clickHouseConfig));
    }
    ...
    public Map<String, ColumnHandle> getColumns(ConnectorSession session, String sql, Map<String, Type> types)
    {
        ...
        try (Connection connection = connectionFactory.openConnection(JdbcIdentity.from(session));
            ...
            logger.warn("in clickhouse push down, clickhouse data source error msg[%s] rewrite sql[%s]", e.getMessage(), sql);
            return Collections.emptyMap();
        }
    }
